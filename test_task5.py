...
tindex = task5.t_index(t)

def test_task5():
    #Output_analytic = ode_solve.xx(inputs)
    #output_numeric =
    Crest_xcoords, Crest_height, tss, tslosh = task5.peak(x, y, t, tindex, ts, 1, 2)

    assert  (Crest_xcoords == [1, 2, 0, 0, 2])
    assert  (Crest_height == [2.0, 8.0, 3.0, 4.0, 5.0])
    assert  (tss == [0, 1, 2, 3, 4])
    assert  (tslosh == 1)
=======
## -*- coding: utf-8 -
#import numpy as np
...
#    assert  (ts == [0, 1, 2, 3, 4])
#    assert  (tslosh == 1)
>>>>>>> 9f282d075ea745c7e12379eb2335db65f45cadc6
